...
if __name__ == "__main__":
    ...
    output_path = 'cache-sqrtl2-sqrtl2'
    ...
    #params_C = {0.00001 : [], 0.0001 : [], 0.001 : [], 0.01 : []}
    #list_C = [0.00001, 0.0001, 0.001, 0.01]
    ...
    for i in range(fold):
        ...
        for C in list_C:
            ...
            params_C[C].append(accuracy)
  
        train_set = validation_set + train_set
        #sdev = np.std()
    
    for j in range(len(params_C)):
        media = np.median(params_C[list_C[j]])
        list_median.append(media)
    
    print('params_C = {}', format(params_C))
