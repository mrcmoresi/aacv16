...
def extract_multiscale_dense_features(imfile, step=8, scales=SCALES_3):
    ...
    for sc in scales:
        ...
        feat = daisy(im_scaled, step=step, normalization='l1')
        if feat.size == 0:
            break
        ndim = feat.shape[2]
        feat = np.atleast_2d(feat.reshape(-1, ndim))

        #Como quiero la norma sqrt => primero normalizo l1 y luego tomo sqrt elemento a elemento
        feat = np.sqrt(feat)
...
def compute_bovw(vocabulary, features, norm=1):
...
if __name__ == "__main__":
    ...
    list_C = [2**-5,2**-3,2**-1,2**1,2**3,2**5,2**7,2**9,2**11,2**13,2**15]
    list_gamma = [2**-15,2**-11,2**-9,2**-7,2**-5,2**-3,2**-1,2**1,2**3]
    ...
    import itertools
    lst_tuples = []
    # cartesian product between params and initialize dict accuracy_params
    for params in itertools.product(list_C, list_gamma):
        accuracy_params[params] = []
        lst_tuples.append(params)

    
    for i in range(fold):
        validation_set = train_set[-w:]
        train_set = train_set[:-w]
        ...
        X_val, y_val = [], []
        for fname, cid in validation_set:
            # cargo los bovw de cada imagen del validation set
            bovwfile = join(output_path, splitext(fname)[0] + '.bovw')
            X_val.append(load_data(bovwfile))
            y_val.append(cid)
        ...
        for C in list_C:
            for gamma in list_gamma:
                svm = SVC(C=C, gamma=gamma, kernel='rbf', verbose=1)
                ...
                accuracy_params[(C, gamma)].append(accuracy)
        train_set = validation_set + train_set

    for j in accuracy_params:
        media = np.median(accuracy_params[j])
        sdev = np.std(accuracy_params[j])
        list_median.append(media)
        lst_sdev.append(sdev)
    
    print('accuracy_params = {}', format(accuracy_params))
    print('list_tuple {}', format(lst_tuples))
    print('list_median = {}', format(list_median))
    print('Accuracy params')
    for index, data in accuracy_params.items():
        print(index, '-->', data)
