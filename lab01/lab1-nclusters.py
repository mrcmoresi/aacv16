...
if __name__ == "__main__":
    ...
    for n_clusters in clusters_size:
        ...
        if exists(vocabulary_file):
            ...
            Utilizo Kmeans++
            k_means = KMeans(n_clusters=n_clusters, init='k-means++', max_iter=100, tol=1e-4)
            k_means_fitted = k_means.fit(sample)
        ...
        #Calculo accuracy y tiempo 
        accuracy = float(tp) / len(y_test)
        elpasedTime = end-start
        
        #agrego a las listas
        lstAcc.append(accuracy)
        lstTime.append(elpasedTime)

    print('clusters size = ', clusters_size)
    print('accuracy = ')
    for i in lstAcc:
        print('%.3f' % i, end=', ') 
    print('list elapsed time = ', format(lstTime))
