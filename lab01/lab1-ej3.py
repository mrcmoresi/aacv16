...
def extract_multiscale_dense_features(imfile, step=8, scales=SCALES_5):
...
if __name__ == "__main__":
    ...
    media_acc = np.median(accuracy_list)
    sdev = np.std(accuracy_list)

    print('accuracy_list ')
    for i in accuracy_list:
        print('%.3f' % i, end=', ') 
    print('media accuracy = {:.4f}'.format(media_acc))
    print('Desviacion standard = {:.4f} accuracy = {:.4f}'.format(sdev, media_acc))
